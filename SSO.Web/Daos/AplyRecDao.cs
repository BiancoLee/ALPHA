...
namespace SSO.Web.Daos
{
    public class AplyRecDao
    {
        ...
        /// <param name="db"></param>
        /// <returns></returns>
        public List<AuthReviewModel> qryAuthReview(string[] unitArry, string authUnit, string cReviewType, string cReviewFlag, dbFGLEntities db)
        {
            ...
            if ("R".Equals(cReviewType))
            {
                authReviewList = (from m in db.SSO_APLY_REC
                                  ...
                                  where (bReviewType || m.APLY_TYPE.Trim() == cReviewType)
                                      ...
                                      & (xRole.AUTH_UNIT == authUnit 
                                            || xRoleM.AUTH_UNIT == authUnit 
                                            || m.CREATE_UNIT == authUnit
            ...
            else if ("U".Equals(cReviewType))
            {
                authReviewList = (from m in db.SSO_APLY_REC
                                  ...
                                  where (bReviewType || m.APLY_TYPE.Trim() == cReviewType)
                                      & (bReviewFlag || m.APPR_STATUS.Trim() == cReviewFlag)
                                      & (m.CREATE_UNIT == authUnit || unitArry.Contains(m.CREATE_UNIT))
                                  select new AuthReviewModel
                                  {
